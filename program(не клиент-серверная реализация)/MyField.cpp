...
Triangulation MyField::generateTriangulation(vector <MyPoint>& pts)
{
	...
	for (unsigned j = 2; j < vertices.size(); j++)
	{
		if (a.getX() * b.getY() - a.getY() * b.getX() < 0)
		{
			...
			T.assingNeiTriangleID(triangulation.findTriangle(p, q, -1));
	...
	{
		flip_ind = 0;
		for (unsigned int v = 0; v < triangulation.getSize() - 1; v++)
		{
			if (triangulation.findTriangle(triangulation.getTriangle(v).getA(), triangulation.getTriangle(v).getB(), v) > -1)
			{
				vv = triangulation.findTriangle(triangulation.getTriangle(v).getA(), triangulation.getTriangle(v).getB(), v);
				aa = triangulation.getTriangle(v).getC();
				bb = triangulation.getTriangle(v).getA();
				cc = triangulation.getTriangle(v).getB();
				if ((triangulation.getTriangle(vv).getA().getID() != bb.getID()) && (triangulation.getTriangle(vv).getA().getID() != cc.getID()))
					dd = triangulation.getTriangle(vv).getA();
				else if ((triangulation.getTriangle(vv).getB().getID() != bb.getID()) && (triangulation.getTriangle(vv).getB().getID() != cc.getID()))
					dd = triangulation.getTriangle(vv).getB();
				else
					dd = triangulation.getTriangle(vv).getC();
				angle1 = acos(scalarCos(aa, bb, cc));
				angle2 = acos(scalarCos(dd, bb, cc));
				if (angle1 + angle2 > Pi)
				{
					triangulation.flipCorrection(Triangle(aa, bb, dd), Triangle(aa, cc, dd), v, vv);
					flip_ind++;
					continue;
				}
			}
			if (triangulation.findTriangle(triangulation.getTriangle(v).getB(), triangulation.getTriangle(v).getC(), v) > -1)
			{
				vv = triangulation.findTriangle(triangulation.getTriangle(v).getB(), triangulation.getTriangle(v).getC(), v);
				aa = triangulation.getTriangle(v).getA();
				bb = triangulation.getTriangle(v).getB();
				cc = triangulation.getTriangle(v).getC();
				if ((triangulation.getTriangle(vv).getA().getID() != bb.getID()) && (triangulation.getTriangle(vv).getA().getID() != cc.getID()))
					dd = triangulation.getTriangle(vv).getA();
				else if ((triangulation.getTriangle(vv).getB().getID() != bb.getID()) && (triangulation.getTriangle(vv).getB().getID() != cc.getID()))
					dd = triangulation.getTriangle(vv).getB();
			...
			if (triangulation.findTriangle(triangulation.getTriangle(v).getA(), triangulation.getTriangle(v).getC(), v) > -1)
			{
				vv = triangulation.findTriangle(triangulation.getTriangle(v).getA(), triangulation.getTriangle(v).getC(), v);
				aa = triangulation.getTriangle(v).getB();
				bb = triangulation.getTriangle(v).getA();
				cc = triangulation.getTriangle(v).getC();
				if ((triangulation.getTriangle(vv).getA().getID() != bb.getID()) && (triangulation.getTriangle(vv).getA().getID() != cc.getID()))
					dd = triangulation.getTriangle(vv).getA();
...
double MyField::scalarCos(MyPoint a, MyPoint b, MyPoint c)
{
	double x1, x2, y1, y2;
	x1 = b.getX() - a.getX(); y1 = b.getY() - a.getY();
	x2 = c.getX() - a.getX(); y2 = c.getY() - a.getY();
